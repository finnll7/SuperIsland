...
struct ShelfExpandedView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 10) {
            HStack(spacing: 8) {
                Label("置物架", systemImage: "tray.full.fill")
                    .font(.system(size: 13, weight: .semibold))
                    .foregroundStyle(.white)

                Text(shelf.items.isEmpty ? "拖放文件、链接、图片或文本" : "已保存 \(shelf.items.count) 项")
            ...
            if totalCount == 0 {
                ...
                VStack(alignment: .leading, spacing: 10) {
                    HStack {
                        ...
                        Text(totalCount == 1 ? "1 项" : "\(totalCount) 项")
...
private struct ShelfItemActionsMenu: View {
    ...
    private var copyTitle: String {
        switch item.kind {
        case .link: return "复制链接"
        case .text: return "复制文本"
        default: return "复制项目"
        }
    }
}
